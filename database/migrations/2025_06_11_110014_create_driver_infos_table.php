...
return new class extends Migration
{
  ...
  public function up(): void
  {
    Schema::create('driver_infos', function (Blueprint $table) {
      ...
      $table->foreignId('user_id')->comment('Reference to the user account')
      ...
      // $table->foreignId('city_id')->comment('City where the driver is registered')
      //   ->references('id')->on('cities')->cascadeOnUpdate()->cascadeOnDelete();

      $table->foreignId('category_id')->comment('Vehicle category')
        ->constrained('categories')->cascadeOnUpdate()->cascadeOnDelete();

      $table->foreignId('brand_id')->comment('Vehicle brand')
        ->constrained('brands')->cascadeOnUpdate()->cascadeOnDelete();

      $table->foreignId('nationality_id')->comment('Driver nationality')
        ->constrained('nationalities')->cascadeOnUpdate()->cascadeOnDelete();


      $table->boolean('appearance_status')
        ->default(false) // Default to false, meaning not available
        ->comment('Driver is currently available or appearing');

      $table->decimal('latitude', 10, 7)->comment('Driver current latitude')->nullable();
      $table->decimal('longitude', 10, 7)->comment('Driver current longitude')->nullable();

      $table->dateTime('recorded_at')->comment('Timestamp of current location update')->nullable();

      $table->string('stcpay_number')->comment('STC Pay number')->nullable();
